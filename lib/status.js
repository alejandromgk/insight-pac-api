...
StatusController.prototype.getInfo = function(callback) {
  this.node.services.bitcoind.getInfo(function(err, result) {
    ...
    var info = {
      ...
      hashrate: result.networkHashRate,
...
StatusController.prototype._getSupplyInfo = function(callback) {
  this.node.services.bitcoind.getSupplyInfo(function(err, info) {
    if (err) {
      return callback(err);
    }
    callback(null, {
      currentSupply: info.currentsupply,
      lockedInMasternodes: info.lockedinmasternodes
    });
  });
};

StatusController.prototype.supplyInfo = function(req, res) {
  var self = this;
  this._getSupplyInfo(function(err, result) {
    if (err) {
      return self.common.handleErrors(err, res);
    }
    res.jsonp(result);
...
StatusController.prototype.currentSupply = function(req, res) {
  var self = this;
  this._getSupplyInfo(function(err, result){
    if (err) {
      return self.common.handleErrors(err, res);
    }
    res.jsonp(result.currentSupply);
  });
};

StatusController.prototype.supplyLockedInMN = function(req, res) {
  var self = this;
  this._getSupplyInfo(function(err, result){
    if (err) {
      return self.common.handleErrors(err, res);
    }
    res.jsonp(result.lockedInMasternodes);
  });
};
