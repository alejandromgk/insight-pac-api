...
GovObjectController.prototype.submitProposal = function(req, res) {
  var self = this;
  var hexdata = req.params.hexdata;
  this.govObjectSubmit(hexdata, function(err, data) {
      if(err) {
          return self.common.handleErrors(err, res);
      }
      res.jsonp(data);
...
GovObjectController.prototype.govObjectSubmit = function(hexdata, callback) {
  this.node.services.bitcoind.govObjectSubmit(hexdata, function(err, result) {
      if (err) {
          return callback(err);
      }
      callback(null, result);
