<h1 align="center">insight-pac-api</h1>

<div align="center">
  <strong>A $PAC blockchain REST and WebSocket API Service</strong>
</div>
<br />
<div align="center">
  <!-- Stability -->
  ...
  <!-- Build Status -->
  <a href="https://travis-ci.org/PACCommunity/insight-pac-api">
    <img src="https://img.shields.io/travis/PACCommunity/insight-pac-api/master.svg?style=flat-square" alt="Build Status" />
  </a>
  <!-- NPM version -->
  <a href="https://npmjs.org/package/insight-pac-api">
    <img src="https://img.shields.io/npm/v/insight-pac-api.svg?style=flat-square" alt="NPM version" />
  </a>
</div>


This is a backend-only service. If you're looking for the web frontend application, take a look at https://github.com/PACCommunity/insight-pac-ui.
...
- [Resources](#resources)
- [License](https://github.com/PACCommunity/insight-pac-api/blob/master/LICENSE)

## Getting Started

```bashl
npm install -g bitcore-node-pac@latest
...
The API endpoints will be available by default at: `http://localhost:3001/insight-pac-api/`

### Prerequisites

- [Bitcore Node $PAC 3.x](https://github.com/PACCommunity/bitcore-node-pac)

**Note:** You can use an existing $PAC data directory, however `txindex`, `addressindex`, `timestampindex` and `spentindex` needs to be set to true in `paccoin.conf`, as well as a few other additional fields.
...
### Block
```
  /insight-pac-api/block/[:hash]
  /insight-pac-api/block/0000000006e7b38e8ab2d351239019c01de9a148b5baef58cfe52dfd9917cedc
```

### Block Index
Get block hash by height
```
  /insight-pac-api/block-index/[:height]
  /insight-pac-api/block-index/0
```
This would return:
```
{
...
### Raw Block
```
  /insight-pac-api/rawblock/[:blockHash]
```
...
```
  /insight-pac-api/blocks?limit=3&blockDate=2017-04-22
```
...
### Transaction
```
  /insight-pac-api/tx/[:txid]
  /insight-pac-api/tx/ebdca263fe1c75c8609ce8fe3d82a320a0b3ca840f4df995883f5dab1b9ff8d9
  /insight-pac-api/rawtx/[:rawid]
  /insight-pac-api/rawtx/ebdca263fe1c75c8609ce8fe3d82a320a0b3ca840f4df995883f5dab1b9ff8d9
```

### Address
```
  /insight-pac-api/addr/[:addr][?noTxList=1][&from=&to=]
  /insight-pac-api/addr/ybi3gej7Ea1MysEYLR7UMs3rMuLJH5aVsW?noTxList=1
  /insight-pac-api/addr/yPv7h2i8v3dJjfSH4L3x91JSJszjdbsJJA?from=1000&to=2000
```

### Address Properties
```
  /insight-pac-api/addr/[:addr]/balance
  /insight-pac-api/addr/[:addr]/totalReceived
  /insight-pac-api/addr/[:addr]/totalSent
  /insight-pac-api/addr/[:addr]/unconfirmedBalance
```
The response contains the value in Satoshis.

### Unspent Outputs
```
  /insight-pac-api/addr/[:addr]/utxo
```
Sample return:
```
[
...
GET method:
```
  /insight-pac-api/addrs/[:addrs]/utxo
  /insight-pac-api/addrs/ygwNQgE5f15Ygopbs2KPRYMS4TcffqBpsz,ygw5yCtVkx3hREke4L8qDqQtnNoAiPKTSx/utxo
```

POST method:
```
  /insight-pac-api/addrs/utxo
```
...
```
  /insight-pac-api/txs/?block=HASH
  /insight-pac-api/txs/?block=000000000814dd7cf470bd835334ea6624ebf0291ea857a5ab37c65592726375
```
### Transactions by Address
```
  /insight-pac-api/txs/?address=ADDR
  /insight-pac-api/txs/?address=yWFfdp9nLUjy1kJczFhRuBMUjtTkTTiyMv
```

### Transactions for Multiple Addresses
GET method:
```
  /insight-pac-api/addrs/[:addrs]/txs[?from=&to=]
  /insight-pac-api/addrs/ygwNQgE5f15Ygopbs2KPRYMS4TcffqBpsz,ygw5yCtVkx3hREke4L8qDqQtnNoAiPKTSx/txs?from=0&to=20
```

POST method:
```
  /insight-pac-api/addrs/txs
```
...
GET method:
```
  /insight-pac-api/gobject/list/proposal
```
...
GET method:
```
  /insight-pac-api/gobject/get/[:hash]
  /insight-pac-api/gobject/get/b6af3e70c686f660541a77bc035df2e5e46841020699ce3ec8fad786f7d1aa35
```
...
### Masternodes List
```
  /insight-pac-api/masternodes/list
```
### Validate Masternode
```
  /insight-pac-api/masternodes/validate/[:payee]
  /insight-pac-api/masternodes/validate/yRuALkPpeYpTgxdNn2L5YgGktASJYDYPAo
```
...
```
  /insight-pac-api/sync
```

### Live Network P2P Data Sync Status
```
  /insight-pac-api/peer
```

### Status of the Bitcoin Network
```
  /insight-pac-api/status?q=xxx
```
...
### Utility Methods
```
  /insight-pac-api/utils/estimatefee[?nbBlocks=2]
```
...
- (Medium)[How to setup a $PAC Instant-Send Transaction using Insight API?????????The comprehensive way](https://medium.com/@obusco/setup-instant-send-transaction-the-comprehensive-way-a80a8a0572e)
